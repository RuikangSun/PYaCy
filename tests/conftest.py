...
def make_mock_suggest_response(queries: list[str] | None = None) -> list[dict[str, str]]:
    """构建模拟的搜索建议响应。
...
@pytest.fixture
def client() -> YaCyClient:
    """提供一个不需要实际网络连接的 YaCyClient 实例。
